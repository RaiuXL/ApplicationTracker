async function fetchUsers() {
	try {
		...
		// Key-coding user options
		const optionsMap = {
			"Seeking Internship" : "I",
			"Seeking Job" : "J",
			"Not Actively Seeking" : "N"
		}

		// Admin Identifiers
		const adminMap = {
			0 : "/pages/Sprint_5/images/isNotAdmin.svg",
			1 : "/pages/Sprint_5/images/isAdmin.svg"
		}
		// 遍历所有筛选后的应用程序数据
		users.forEach((app) => {
			const appDiv = document.createElement("div");
			const currUserOptions = app.appUseOptions.split(", ");
			let keyCode = "";
			currUserOptions.map((curr) => {
				keyCode += optionsMap[curr];
			})
			appDiv.innerHTML = `
